...
class TestAlignAtMaximumCorrelation:
    ...
    def setup_method(self):
        ...
        base_ts = AR1_process(rho=0.8, sigma=1.0, y0=0.0, N=self.N + 20, seed=42, return_xarray=False)
        ...
        self.ts2 = TimeSeries(base_ts.time[:self.N], base_ts.data[self.lag_offset:self.N+self.lag_offset], self.dt)
    ...
    def test_xarray_inputs(self):
        ...
        ts1_aligned, ts2_aligned = align_at_maximum_correlation(
            ts1_xr, ts2_xr, maxlags=self.maxlags
        )
    ...
    def test_mixed_inputs(self):
        """Test alignment with mixed TimeSeries and xarray inputs."""
        ts2_xr = self.ts2.to_xarray()

        # TimeSeries + xarray
        ts1_aligned, ts2_aligned = align_at_maximum_correlation(
            self.ts1, ts2_xr, maxlags=self.maxlags
        )
    ...
    def test_negative_lag_alignment(self):
        ...
        ts2_leads = TimeSeries(
            self.ts1.time,
            np.concatenate([self.ts1.data[3:], np.zeros(3)]),  # ts2 leads by 3 steps
            self.dt
        )
    ...
    def test_maxlags_parameter(self):
        ...
        assert corr_small > 0.9  # Both should find good alignment for shifted data
        assert corr_large > 0.9
        # Large maxlags should find better or equal alignment
        assert corr_large >= corr_small - 0.1  # Allow small tolerance
    ...
    def test_short_series_alignment(self):
        ...
        short_ts1 = TimeSeries(np.arange(10), np.random.randn(10), 1.0)
        short_ts2 = TimeSeries(np.arange(10), np.random.randn(10), 1.0)
    ...
    def test_alignment_preserves_attributes(self):
        ...
        ts1_aligned, ts2_aligned = align_at_maximum_correlation(ts1_xr, ts2_xr)
    ...
    def test_time_coordinate_validation(self):
        ...
        with pytest.raises(ValueError, match="ts1 must have 'time' coordinate"):
            align_at_maximum_correlation(data_no_time, ts2_xr)
        ...
        with pytest.raises(ValueError, match="Time coordinates must be numeric"):
            align_at_maximum_correlation(ts1_xr, ts_bad_time)
        ...
        with pytest.raises(ValueError, match="Time series must have same length"):
            align_at_maximum_correlation(ts1_xr, ts_short)

        # Test misaligned time coordinates
        ts_misaligned = xr.DataArray(
            np.random.randn(len(ts1_xr)),
            coords={"time": np.arange(len(ts1_xr)) + 10},  # Offset by 10
            dims=["time"]
        )

        with pytest.raises(ValueError, match="Time coordinates must be aligned"):
            align_at_maximum_correlation(ts1_xr, ts_misaligned)
...
class TestDeprecatedShiftMaximallyCorrelated:
    ...
    def test_deprecated_function_warning(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            ts1_aligned, ts2_aligned = shift_maximally_correlated(ts1, ts2, maxlags=10)
    ...
    def test_deprecated_function_equivalent_results(self):
        ...
        with warnings.catch_warnings():
            ...
            ts1_old, ts2_old = shift_maximally_correlated(ts1, ts2, maxlags=10)

        # Get results from new function (always returns xarray, so convert for comparison)
        ts1_new_xr, ts2_new_xr = align_at_maximum_correlation(ts1, ts2, maxlags=10)
